...
function ReactFlowGraph({ nodes, edges, onPick, viewMode, selection, selectedComponentType }) {
  ...
  const reactFlowNodes = useMemo(() => {
    ...
    const getImpactState = (node) => {
      ...
      // If a component type is selected, show that type as "down" and connected nodes as "impacted"
      if (selectedComponentType) {
        if (nodeComponentType === selectedComponentType) return 'selected';

        // Check if this node is connected to any node of the selected component type
        const isConnectedToSelectedType = edges.some(edge => {
          ...
          return (
            (edge.src === node.id && dstComponentType === selectedComponentType) ||
            (edge.dst === node.id && srcComponentType === selectedComponentType)
          );
        });

        if (isConnectedToSelectedType) return 'impacted';
      }

      // Individual node selection (original behavior)
      if (selection && selection.type === 'node') {
        const selectedNodeId = selection.data.id;
        if (node.id === selectedNodeId) return 'selected';

        // Check if this node is upstream or downstream of selected node
        ...
        if (isUpstream || isDownstream) return 'impacted';
      }
    ...
    return layoutNodes;
  }, [nodes, edges, selection, selectedComponentType]);
...
export default function ImpactMode() {
  ...
  // Filter state - single component type selection for impact analysis
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ border: "1px solid #e5e7eb", background: "#fff", borderRadius: 16, boxShadow: "0 1px 2px rgba(0,0,0,0.04)" }}>
            <div style={{ padding: "8px 16px", borderBottom: "1px solid #e5e7eb", fontSize: 14, fontWeight: 600 }}>Select Components to see impact</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 13 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input
                    type="radio"
                    name="componentType"
                    checked={selectedComponentType === null}
                    onChange={() => setSelectedComponentType(null)}
                  />
                  All
                </label>
                {['api-gateway', 'service', 'kestrel', 'pulsar', 'oracle', 'redis'].map(componentType => (
                  <label key={componentType} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <input
                      type="radio"
                      name="componentType"
                      checked={selectedComponentType === componentType}
                      onChange={() => setSelectedComponentType(componentType)}
                    />
                    {componentType}
                  </label>
                ))}
