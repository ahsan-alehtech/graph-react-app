...
import impactAnalysisData from './data/impactAnalysisData.json';

/**
 * Impact Mode UI (OTel‑powered)
...
function convertServiceGraphData(serviceData) {
  ...
  const nodes = serviceData.nodes.map(node => ({
    id: node.id,
    label: `${node.id} (${node.componentType})`, // Show ID with componentType in parentheses
...
const CircularNodeWithHandles = ({ data, selected }) => {
  ...
  const isSelectedNode = data.impactState === 'selected';
  const isImpactedNode = data.impactState === 'impacted';

  let nodeColor = '#1e40af'; // Default blue
  let borderColor = '#1e40af';

  if (isSelectedNode) {
    nodeColor = '#dc2626'; // Dark red for selected node
    borderColor = '#dc2626';
  } else if (isImpactedNode) {
    nodeColor = '#f87171'; // Light red for impacted nodes
    borderColor = '#f87171';
  }
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{
        ...
        color: 'white',
        border: selected ? `3px dashed ${borderColor}` : `2px solid ${borderColor}`,
        borderRadius: '50%',
        padding: '4px',
        width: isEntryPoint ? '20px' : '16px',
        height: isEntryPoint ? '20px' : '16px',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        boxShadow: selected ? `0 0 0 2px white, 0 0 0 4px ${borderColor}` : 'none',
        position: 'relative'
      }}></div>
...
function ReactFlowGraph({ nodes, edges, onPick, viewMode, selection, impactModeFilters, selectedComponentTypes }) {
  ...
  const reactFlowNodes = useMemo(() => {
    ...
    // Define positions for our comprehensive microservices architecture
    const nodePositions = {
      // Layer 1: Client Entry Points (Top)
      'web-app': { x: centerX - 200, y: 50 },
      'mobile-api': { x: centerX, y: 50 },
      'admin-portal': { x: centerX + 200, y: 50 },

      // Layer 2: API Gateway (Entry)
      'api-gateway': { x: centerX, y: 150 },

      // Layer 3: Core Services (Business Logic)
      'auth-service': { x: centerX - 300, y: 250 },
      'user-service': { x: centerX - 150, y: 250 },
      'profile-service': { x: centerX, y: 250 },
      'cart-service': { x: centerX + 150, y: 250 },
      'catalog-service': { x: centerX + 300, y: 250 },

      // Layer 4: Business Services
      'order-service': { x: centerX - 200, y: 350 },
      'payment-service': { x: centerX - 50, y: 350 },
      'inventory-service': { x: centerX + 100, y: 350 },
      'search-service': { x: centerX + 250, y: 350 },

      // Layer 5: Supporting Services
      'shipping-service': { x: centerX - 250, y: 450 },
      'notification-service': { x: centerX - 100, y: 450 },
      'analytics-service': { x: centerX + 50, y: 450 },
      'recommendation-service': { x: centerX + 200, y: 450 },
      'review-service': { x: centerX + 350, y: 450 },

      // Layer 6: Event Streams (Messaging)
      'order-events': { x: centerX - 300, y: 550 },
      'payment-events': { x: centerX - 150, y: 550 },
      'user-events': { x: centerX, y: 550 },
      'notification-queue': { x: centerX + 150, y: 550 },
      'analytics-events': { x: centerX + 300, y: 550 },
      'inventory-events': { x: centerX + 450, y: 550 },

      // Layer 7: Workers (Background Processing)
      'payment-processor': { x: centerX - 250, y: 650 },
      'email-worker': { x: centerX - 100, y: 650 },
      'sms-worker': { x: centerX + 50, y: 650 },
      'analytics-worker': { x: centerX + 200, y: 650 },
      'image-processor': { x: centerX + 350, y: 650 },
      'report-generator': { x: centerX + 500, y: 650 },

      // Layer 8: Databases (Data Layer)
      'user-db': { x: centerX - 300, y: 750 },
      'order-db': { x: centerX - 150, y: 750 },
      'payment-db': { x: centerX, y: 750 },
      'analytics-db': { x: centerX + 150, y: 750 },
      'catalog-db': { x: centerX + 300, y: 750 },
      'inventory-db': { x: centerX + 450, y: 750 },

      // Layer 9: Caches (Fast Data Layer)
      'session-cache': { x: centerX - 250, y: 850 },
      'user-cache': { x: centerX - 100, y: 850 },
      'product-cache': { x: centerX + 50, y: 850 },
      'recommendation-cache': { x: centerX + 200, y: 850 },
      'search-cache': { x: centerX + 350, y: 850 },
      'cart-cache': { x: centerX + 500, y: 850 }
    };

    // Calculate impact state for each node
    const getImpactState = (node) => {
      const nodeComponentType = node.attrs?.componentType || node.type;

      // Component type selection - show selected types as "down" and connected nodes as "impacted"
      if (selectedComponentTypes.length > 0) {
        if (selectedComponentTypes.includes(nodeComponentType)) return 'selected';

        // Check if this node is connected to any node of the selected component types
        const isConnectedToSelectedTypes = edges.some(edge => {
          const srcNode = nodes.find(n => n.id === edge.src);
          const dstNode = nodes.find(n => n.id === edge.dst);
          const srcComponentType = srcNode?.attrs?.componentType || srcNode?.type;
          const dstComponentType = dstNode?.attrs?.componentType || dstNode?.type;

          // Upstream: this node feeds data TO selected types (this node → selected type)
          const isUpstreamConnection = edge.src === node.id && selectedComponentTypes.includes(dstComponentType);
          // Downstream: selected types feed data TO this node (selected type → this node)
          const isDownstreamConnection = edge.dst === node.id && selectedComponentTypes.includes(srcComponentType);

          // Apply impact mode filters
          return (isUpstreamConnection && impactModeFilters.upstream) ||
                 (isDownstreamConnection && impactModeFilters.downstream);
        });

        if (isConnectedToSelectedTypes) return 'impacted';
      }

      // Individual node selection with impact mode filtering
      if (selection && selection.type === 'node') {
        const selectedNodeId = selection.data.id;
        if (node.id === selectedNodeId) return 'selected';

        // Check if this node is upstream or downstream of selected node based on filters
        // Upstream: this node feeds data TO the selected node (this node → selected node)
        const isUpstream = edges.some(edge => edge.src === node.id && edge.dst === selectedNodeId);
        // Downstream: selected node feeds data TO this node (selected node → this node)
        const isDownstream = edges.some(edge => edge.src === selectedNodeId && edge.dst === node.id);

        // Apply impact mode filters
        const showUpstream = impactModeFilters.upstream;
        const showDownstream = impactModeFilters.downstream;

        if ((isUpstream && showUpstream) || (isDownstream && showDownstream)) {
          return 'impacted';
        }
      }
    ...
    const layoutNodes = nodes.map((node) => {
      ...
      return {
        ...
        data: {
          ...
          originalNode: node,
          impactState: getImpactState(node)
        },
      };
    });

    return layoutNodes;
  }, [nodes, edges, selection, impactModeFilters, selectedComponentTypes]);
...
export default function ImpactMode() {
  ...
  const seed = useMemo(() => convertServiceGraphData(impactAnalysisData), []);
  ...
  // Impact mode filter state - both upstream and downstream selected by default
  const [impactModeFilters, setImpactModeFilters] = useState({
    upstream: true,
    downstream: true
  });

  // Component type filter state
  ...
  // Available component types from the data
  const availableComponentTypes = ['api-gateway', 'service', 'kestrel', 'pulsar', 'oracle', 'redis'];
  ...
  const derived = useMemo(() => {
    ...
    // Apply componentType filters - show all nodes always
    const componentFilteredNodes = filteredNodes;
    ...
    return { nodes, edges };
  }, [editing, working, maps, feature, viewMode, flowActive, flowResult, selectedComponents]);
  ...
  return (
    <div style={outerStyle}>
      ...
      <div style={{ padding: "12px 24px", borderBottom: "1px solid #e5e7eb", background: "#fff", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 18, fontWeight: 600 }}>NexusNova - Impact Mode</div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>
                {selection && selection.type === "node"
                  ? `Select node to see impact if ${selection.data.label} goes down`
                  : selectedComponents.length > 0
                  ? `Viewing ${selectedComponents.length} selected components`
                  : "Select node to see impact if that goes down"
                }
              </div>
            </div>
          </div>
      ...
      <div style={{
        display: "grid",
        gridTemplateColumns: "0.72fr 2fr",
        gap: 16,
        padding: 16,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ border: "1px solid #e5e7eb", background: "#fff", borderRadius: 16, boxShadow: "0 1px 2px rgba(0,0,0,0.04)" }}>
            <div style={{ padding: "8px 16px", borderBottom: "1px solid #e5e7eb", fontSize: 14, fontWeight: 600 }}>Filter</div>

            {/* Impact Mode Filters */}
            <div style={{ padding: 16, borderBottom: "1px solid #e5e7eb" }}>
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8, color: '#374151' }}>Impact Mode</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 13 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input
                    type="checkbox"
                    checked={impactModeFilters.upstream}
                    onChange={(e) => setImpactModeFilters(prev => ({ ...prev, upstream: e.target.checked }))}
                  />
                  Upstream
                </label>
                <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input
                    type="checkbox"
                    checked={impactModeFilters.downstream}
                    onChange={(e) => setImpactModeFilters(prev => ({ ...prev, downstream: e.target.checked }))}
                  />
                  Downstream
                </label>
              </div>
            ...
            <div style={{ padding: 16 }}>
                ...
                <div style={{ marginBottom: 8, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {selectedComponentTypes.map(type => (
                    ...
                    >
                      {type}
                      <button
                        onClick={() => setSelectedComponentTypes(prev => prev.filter(t => t !== type))}
                        style={{
                          background: 'none',
                          border: 'none',
                          color: '#1e40af',
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  onChange={(e) => {
                    setComponentTypeSearch(e.target.value);
                    setShowComponentTypeDropdown(true);
                  }}
                  onFocus={() => setShowComponentTypeDropdown(true)}
                  onBlur={() => setTimeout(() => setShowComponentTypeDropdown(false), 200)}
                ...
                {showComponentTypeDropdown && (
                  <div style={{
                    ...
                    {availableComponentTypes
                      .filter(type =>
                        type.toLowerCase().includes(componentTypeSearch.toLowerCase()) &&
                      ...
                      .map(type => (
                        <div
                          key={type}
                          onClick={() => {
                            setSelectedComponentTypes(prev => [...prev, type]);
          ...
          <div style={{ border: "1px solid #e5e7eb", background: "#fff", borderRadius: 16, boxShadow: "0 1px 2px rgba(0,0,0,0.04)", padding: 16 }}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Impact Legend</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 12, color: '#374151' }}>Down Node</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{
                  width: 16,
                ...
                <span style={{ fontSize: 12, color: '#374151' }}>Impacted Node</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{
                  width: 16,
                ...
                <span style={{ fontSize: 12, color: '#374151' }}>Normal Node</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ padding: "8px 16px", borderBottom: "1px solid #e5e7eb", fontSize: 14, fontWeight: 600 }}>Impact Mode</div>
          <div style={{ flex: 1 }}>
            <ReactFlowGraph
              nodes={derived.nodes}
              edges={derived.edges}
              viewMode={viewMode}
              selection={selection}
              impactModeFilters={impactModeFilters}
